...
// Dual frame buffers for ping-pong (double buffering)
uint16_t* frameBufferA = nullptr;
uint16_t* frameBufferB = nullptr;
uint16_t* frameBuffer = nullptr; // currently rendering buffer (core 1)
uint16_t* readyBuffer = nullptr; // ready for encoding (core 0)
...
// Frame synchronization for dual-core ping-pong
SemaphoreHandle_t frameReadySem;  // signals core 0 that a new frame is ready
SemaphoreHandle_t frameFreeSem;   // signals core 1 that core 0 finished with buffer
...
// Forward declaration for core 0 web task
...
void drawThermalFrame() {
  if (!readyBuffer) return;  // no frame ready yet
  ...
  gfx->draw16bitRGBBitmap(startX, startY, readyBuffer, VIEW_W, VIEW_H);
...
void setup() {
  ...
  // Allocate dual frame buffers (use PSRAM if available)
  if (psramFound()) {
    frameBufferA = (uint16_t*)ps_malloc(VIEW_W * VIEW_H * sizeof(uint16_t));
    frameBufferB = (uint16_t*)ps_malloc(VIEW_W * VIEW_H * sizeof(uint16_t));
    Serial.printf("PSRAM found: %d bytes free\n", ESP.getFreePsram());
    Serial.println("Running dual-core mode with PSRAM");
  } else {
    frameBufferA = (uint16_t*)malloc(VIEW_W * VIEW_H * sizeof(uint16_t));
    frameBufferB = (uint16_t*)malloc(VIEW_W * VIEW_H * sizeof(uint16_t));
    Serial.println("PSRAM not found, using heap");
    Serial.println("Running dual-core mode without PSRAM");
  }
  ...
  frameBuffer = frameBufferA;  // core 1 starts rendering to A
  readyBuffer = nullptr;       // no frame ready yet

  initTempPalette();
  initResampleMaps();

  // Create semaphores for ping-pong buffer handoff
  frameReadySem = xSemaphoreCreateBinary();
  frameFreeSem = xSemaphoreCreateBinary();
  if (frameReadySem == NULL || frameFreeSem == NULL) {
    Serial.println("Failed to create semaphores!");
    while (1) delay(100);
  }
  xSemaphoreGive(frameFreeSem); // initially core 1 can render
  ...
  // Spawn web server task on core 0
  if (wifiManIsConnected()) {
  ...
  Serial.println("Setup complete! Dual-core mode (core 1: capture/render/UI, core 0: web/encode).");
}
...
void loop() {
  ...
  // Wait for core 0 to finish with previous buffer
  if (xSemaphoreTake(frameFreeSem, pdMS_TO_TICKS(100)) == pdTRUE) {
    ...
    // Swap buffers: readyBuffer becomes what we just rendered
    readyBuffer = frameBuffer;
    frameBuffer = (frameBuffer == frameBufferA) ? frameBufferB : frameBufferA;

    // Signal core 0 that a new frame is ready
    xSemaphoreGive(frameReadySem);
  }

  // Draw current frame to the display (use readyBuffer so display shows last complete frame)
  if (readyBuffer) {
    drawThermalFrame();
  }
...
// Core 0 task: web server + JPEG encoding
void webServerTask(void* parameter) {
  webTaskRunning = true;
  Serial.println("[Core 0] Web task running");
